...
int main(int argc, char** argv){
   ...
   // MPI initialization
    ...
    backend_options[backend_option_niter] = 10; // default number of iterations

    fiber_parse_options(argc, argv, backend_options, lib_name, lib_1d_backend);
    backend_options[backend_option_fft_op] = 0;  // Fixed to benchmark a forward FFT computation
    backend_options[backend_option_backend] = fiber_get_1d_backend(lib_1d_backend);
    ...
    // Global grid
    int nx, ny, nz ;
    nx = backend_options[backend_option_nx];
    ny = backend_options[backend_option_ny];
    nz = backend_options[backend_option_nz];
    ...
    int proc_grid[2] = {0,0};
    proc_grid[0] = backend_options[backend_option_grid_p];
    proc_grid[1] = backend_options[backend_option_grid_q];

    if (proc_grid[0]==0 || proc_grid[1]==0){
        MPI_Dims_create(num_ranks, 2, proc_grid);
        backend_options[backend_option_grid_p] = proc_grid[0];
        backend_options[backend_option_grid_q] = proc_grid[1];
    }
   ...
   printf("[%d] proc_grid 1x%dx%d - [New MPI process %d] I am located at (%d, %d).   %d-%d-%d    %d-%d-%d   local_size={%d} \n", me, proc_grid[0], proc_grid[1],  proc_rank, proc_coords[0],proc_coords[1], \
    ...
    if (me==0){
        ...
        printf("Time for execution = %10.6g \n", timer[1]/backend_options[backend_option_niter]);
    }
    ...
    backend_options[backend_option_fft_op] = 1; // Setting to backward for error validation
